...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/10 19:55:05 by asmall            #+#    #+#             */
/*   Updated: 2020/02/10 22:33:35 by asmall           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	take_stack_op(t_stack **a, t_stack **b, t_op *val, char *opt)
{
	...
	choose_op(a, b, opt);
	if (val->v_flag == 1 || val->v_flag == 2)
		(val->v_flag == 1) ? ft_v(*a, *b, val, opt) : ft_vz(*a, *b, val, opt);
	else
		ft_printf("%s\n", opt);
}
